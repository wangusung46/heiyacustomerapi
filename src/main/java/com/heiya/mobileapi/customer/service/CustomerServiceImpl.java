...
public class CustomerServiceImpl implements CustomerService {
    ...
    public BaseResponse performCustomerRegistration(CustomerRegistrationDTORequest request) {
        ...
        try {
            ...
            if (customerVal1 != null) {
                //response
                if ("Y".equals(customerVal1.getIsActive())) {
                    response.setSuccess(false);
                    response.setResultCode("412");
                    ...
                    customerVal1.setFirstName(request.getFirstName());
                    customerVal1.setLastName(request.getLastName());
                    customerVal1.setGender(request.getGender());
                    customerVal1.setDob(request.getDob());
                    customerVal1.setEmail(request.getEmail());
                    customerVal1.setAddress(request.getAddress());
                    BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder();
                    customerVal1.setPassword(passwordEncoder.encode(request.getPassword()));
                    customerVal1.setIsActive(request.getIsActive());
                    customerRepo.save(customerVal1);

                    //response
                    response.setSuccess(true);
                    response.setResultCode("200");
                    response.setResultMsg("Registration success");
                    LOGGER.info("======== CustomerServiceImpl.performCustomerRegistration() - Registration with email " + customerVal1.getEmail() + " success");
                    return response;
                }
            }
    ...
    public CustomerDetailDTOResponse performCustomerInquiry(Long customerId) throws Exception {
        ...
        if (customer != null) {
            if ("Y".equals(customer.get().getIsActive())) {
                response.setSuccess(true);
                response.setResultCode("200");
...
//        String onDevice = "IOS";
    ...
    public BaseResponse performDeleteAccount(String mobileNo) throws Exception {
        BaseResponse response = new BaseResponse();
        LOGGER.info("======== START CustomerServiceImpl.performDeleteAccount() with Phone Number : " + mobileNo);
        if (mobileNo.startsWith("+62")) {
            mobileNo = mobileNo.replace("+62", "0");
        }
        Customer customer = customerRepo.findByMobileNo(mobileNo);
        if (customer != null) {
            if (customer.getIsActive().equals("Y")) {
                ...
                response.setSuccess(true);
                response.setResultCode("200");
                response.setResultMsg("Successfully deleted customer data");
                LOGGER.info("======== CustomerServiceImpl.performDeleteAccount() - Customer found :" + mapper.writeValueAsString(response));
            } else {
                response.setSuccess(false);
                response.setResultCode("404");
                response.setResultMsg("Customer not actived");
                LOGGER.info("======== CustomerServiceImpl.performDeleteAccount() - Customer not actived");
            }
        } else {
            response.setSuccess(false);
            response.setResultCode("404");
            response.setResultMsg("Customer not found");
            LOGGER.info("======== CustomerServiceImpl.performDeleteAccount() - Customer not found");
        }
        return response;
    }
    ...
    public BaseResponse performUnDeleteAccount(String mobileNo) throws Exception {
        BaseResponse response = new BaseResponse();
        LOGGER.info("======== START CustomerServiceImpl.performUnDeleteAccount() with Phone Number : " + mobileNo);
        if (mobileNo.startsWith("+62")) {
            mobileNo = mobileNo.replace("+62", "0");
        }
        Customer customer = customerRepo.findByMobileNo(mobileNo);
        if (customer != null) {
            if (customer.getIsActive().equals("N")) {
                ...
                response.setSuccess(true);
                response.setResultCode("200");
                response.setResultMsg("Successfully un-deleted customer data");
                LOGGER.info("======== CustomerServiceImpl.performUnDeleteAccount() - Customer found :" + mapper.writeValueAsString(response));
            } else {
                response.setSuccess(false);
                response.setResultCode("404");
                response.setResultMsg("Customer actived");
                LOGGER.info("======== CustomerServiceImpl.performUnDeleteAccount() - Customer actived");
            }
        } else {
            response.setSuccess(false);
            response.setResultCode("404");
            response.setResultMsg("Customer not found");
            LOGGER.info("======== CustomerServiceImpl.performUnDeleteAccount() - Customer not found");
        }
        return response;
    }
}
